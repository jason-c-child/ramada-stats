      ...
      <div className="win95-window">
        <div className="win95-title-bar flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm sm:text-base">Validator Statistics</span>
          </div>
      ...
      <div className="p-3 sm:p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="win95-window-inset p-3 sm:p-4">
            ...
            <div className="text-black text-xs sm:text-sm font-bold mb-1">Avg Voting Power</div>
            <div className="text-lg sm:text-xl md:text-2xl font-bold text-black mb-1">
        ...
        <div className="win95-window-inset p-3 sm:p-4">
          <h3 className="text-base sm:text-lg font-bold text-black mb-3 sm:mb-4">Top 10 Validators by Voting Power</h3>
          <div className="overflow-x-auto">
            <table className="win95-table w-full min-w-[600px]">
              <thead>
                <tr>
                  <th className="text-xs sm:text-sm">Rank</th>
                  <th className="text-xs sm:text-sm">Validator</th>
                  <th className="text-xs sm:text-sm">Voting Power</th>
                  <th className="text-xs sm:text-sm">Commission</th>
              ...
              <tbody>
                {stats.topValidators.map((validator, index) => (
                  <tr key={validator.address}>
                    <td className="font-bold text-xs sm:text-sm">#{index + 1}</td>
                    <td>
                      <div>
                        <div className="font-bold text-black text-xs sm:text-sm">{validator.name}</div>
                        <div className="text-xs text-black font-mono">{validator.address.slice(0, 12)}...</div>
                      </div>
                    </td>
                    <td className="font-bold text-black text-xs sm:text-sm">
                      {formatVotingPower(validator.voting_power)}
                    </td>
                    <td className="text-black text-xs sm:text-sm">{validator.commission}</td>
                    <td>
                      <span className={`inline-flex items-center px-1 sm:px-2 py-1 text-xs font-bold ${
